...
test('Test Remove Resource/Role', function () {
  ...
  for (var i = 0; i < resources.length; i++) {
    for (var j = 0; j < roles.length; j++) {
      ...
      expect(A.isAllowed(new Role(roles[j]), new Resource(resources[i]))).toBe(
        true
      );
    }
  }
...
test('Test Export/Import', function () {
  ...
  expect(
    Object.keys(newRes.registry).length > Object.keys(resources.registry).length
  ).toBe(true); // Exported resources is unaffected
  expect(
    Object.keys(newRoles.registry).length > Object.keys(roles.registry).length
  ).toBe(true); // Exported roles is unaffected
